...
export function DeviceMockup() {
	...
	const OutgoingBubble = ({ msg }: { msg: BuiltMessage }) => {
		...
		return (
			<div
				className={`message-bubble group relative ml-8 rounded-lg border border-primary/30 bg-primary/20 p-2 ${isNewMessage(msg.id)
	...
	const IncomingBubble = ({ msg }: { msg: BuiltMessage }) => {
		...
		return (
			<div
				className={`message-bubble group relative flex gap-2 rounded-lg bg-gray-800/50 p-2 ${isNewMessage(msg.id)
					? "animate-in slide-in-from-bottom-4 fade-in duration-300"
					: ""
					}`}
			>
				<div
					className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-full ${isEmojiAvatar ? "text-2xl" : "text-xs font-bold"
						}`}
					style={
						isEmojiAvatar
						...
						<div className="bg-gray-900/50 px-4 pb-3 pt-4">
							<div className="flex items-center gap-2 rounded-full border-0 bg-gray-800 px-4 py-2.5 shadow-none">
								...
								<button
									...
									className={`flex h-7 w-7 items-center justify-center rounded-full border-0 shadow-none transition-colors ${inputValue.trim()
										...
										<Fragment key={key}>
											{key === "😊" ? (
												...
												<button
													...
													className={`flex items-center justify-center rounded border-0 bg-gray-800/50 font-mono text-xs font-medium text-gray-200 shadow-none transition-colors hover:bg-primary hover:text-gray-900 active:scale-95 ${key === "space"
														? "h-7 flex-1"
														: key === "↵"
