...
date: 2025-09-18T16:42+1200
hide_table_of_contents: false
---

Since the last [guide on choosing the right role][1], a new role - ROUTER_LATE
- has been added. What exactly does this role do, when should it be used, and
how does it fit into the overall picture of how a Meshtastic network operates?
...
the IP networking world. Instead, Meshtastic uses a combination of role
selection, random delays, and timing based on the signal strength of a received
packet to ensure that any given packet will be more likely to take a more
efficient path across the network, while minimising unnecessary rebroadcasts
that can add to congestion on the single frequency that is shared by the entire
mesh.
