...
date: 2025-11-13T12:00
hide_table_of_contents: false
image: /img/blog/zero_cost_hopscotch.webp
---
...
![Blursed image of a hopscotch game with multiple 3s and 4s written on the squares](/img/blog/zero_cost_hopscotch.webp)

## TL;DR

This feature allows a mesh to add more hops for messages between routers without burning hops.
...
- Helps avoid `CLIENT_BASE` from being the last hop of a packet

:::note  
> Favoriting your local router from your roof CLIENT_BASE node means incoming packets don’t “die on the roof" before reaching your indoor nodes.
:::
...
3. **Previous relay** is in your *favorites* **and** holds and is `ROUTER` or `ROUTER_LATE`  
...
| Nodes | Favorites | Collision Chance |
|-------|-----------|------------------|
| 10    | 1         | ~3.5 % |
| 50    | 3         | ~43 % |
| 100   | 5         | ~84 % |
...
In practice, chances of a collision also being a router is rare.
...
- Routers **R1** and **R2** are infrastructure and favorited (on each other) 
- Someone (**CLIENT**) wants to chat with a friend in the next town
...
- Roof node is `CLIENT_BASE`.  
- Hilltop `ROUTER`s are favorited.
...
**Takeaway:** This demontrates the ability to go beyond the 7 hop protocol limit
...
For Bay Mesh, there are over 700 nodes between Tahoe and San Louis Obispo. With careful placement of routers it is possible to have messages span over 300 miles that can only be accomplished on LoRa using zero-cost hops given the terrain and distance.

Using [Meshview](https://github.com/pablorevilla-meshtastic/meshview) hosted by [bayme.sh](https://meshview.bayme.sh) you can see the zero-cost hops in action between San Francisco, San Louis Obispo, and Tahoe.
...
Pick two or three strategic infrastructure nodes. favorite them to each other mutually. 
