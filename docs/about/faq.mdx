...
export const Faq = {
  ...
  "firmware": [
    ...
    },
    {
      ...
      content: `Firmware should be kept up to date. While it is not necessary to update whenever a new release comes out it's a good idea to follow the Discord Server and Forums linked in the 
[Introduction](/docs/introduction/#contributors) where developers may advise the community about important updates.  The Meshtastic [firmware releases page](https://github.com/meshtastic/firmware/releases)` on Github has a summary of what has changed in each release.
    },
  ],
  "android": [
    {
