...
export const Faq = {
  ...
  - Cloud with a check mark - At least one other node on the mesh acknowledged the message and retransmitted it. Note that you should only consider the message delivered if it was acknowledged by the recipient.
  - Person with a check mark - The intended recipient of your direct message acknowledged the message (whether or not the mesh was used).
