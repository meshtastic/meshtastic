...
**Recommendations** for users using _private channels_:
...
- Direct Messages are now protected with PKI when messaging between devices on v2.5.0 or newer and key exchange has taken place, messaging to legacy 2.4.3 or older firmware versions are not protected.
...
Meshtastic does not check channel messages have not been tampered with, see [`#4030`](https://github.com/meshtastic/firmware/issues/4030) for details.
