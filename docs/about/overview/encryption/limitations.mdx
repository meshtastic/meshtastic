---
id: limitations
title: Known Limitations of Meshtastic's Encryption
sidebar_label: Limitations
sidebar_position: 3
description: "Notes on the Meshtastic encryption and security model, exploring the known limitations that result from the various requirements of the project."
---

import QRCode from "@site/docs/blocks/_qr-code.mdx";

Meshtastic’s security model sits at the intersection of multiple conflicting requirements. This document is an attempt to explain those requirements, the tradeoffs they represent, and characterise the current limitations chosen to make Meshtastic usable. It will also delve into recent changes and upcoming fixes.
...
The second major mitigation is that nodes that are marked as favorite are never dropped from the NodeDB. Clients have recently started automatically marking nodes as favorite when a DM is sent to that node, further ensuring that the node you were chatting with is still the same node.
...
Packets sent from a connected client may set the “pki_encrpyted” boolean to true, and populate the public_key byte field on a packet sent through the local API. If the boolean is set to true, then the packet will only be sent via a PKI DM. If the bytes are populated, the packet will only be sent if the public key in the API message matches the public key for that node in the local NodeDB. If the boolean is not set to true, the firmware will send the packet as a PKI DM if it is indeed sent to a single target, and the firmware has a public key for that target. Outgoing messages will fall back to using the less secure channel encryption if there is not a public key known for the remote node and the pki_encrypted bool is unset
...
Once this bit is set, unsigned user packets from this source will be dropped. To be determined is how other unsigned packets will be treated. At this point, I believe that short packets, that could be signed, should be dropped when unsigned.
