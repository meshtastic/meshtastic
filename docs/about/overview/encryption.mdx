...
Meshtastic's encryption is not threatened by Quantum-Computers\*, so realistic attack vectors are accidental mishandling of channel keys and nodes being lost.

\*On the Quantum-Resistance of AES256, see:
- [this stackoverflow question, containing a bit of debate and two good answers](https://crypto.stackexchange.com/q/6712)
...
**Changes since 2.5 Firmware release**
- Direct Messages are now protected when messaging between devices on 2.5 or newer, messaging to legacy 2.4 or older are not protected.
- Admin Messages between two 2.5 or newer nodes are now protected and additonal relay attack protection.
### Integrity

Integrity means ensuring messages cannot be modified without the key.

Meshtastic does not check messages have not been tampered with, see [`#4030`](https://github.com/meshtastic/firmware/issues/4030) for details.
...
Direct Messages (DMs) are now implemented using Public Key Infrastructure (PKI). This provides a significant upgrade in security compared to the previous channel-based encryption:
...
**Prior to 2.5 Firmware Release**

Previously, DMs were implemented as channel messages with a `to` protobuf field set. This meant anyone in the channel could read all your direct messages. With PKI, this issue is resolved, as only the intended recipient can decrypt and read the message.

**Recommendations**:
- While PKI significantly enhances the security of DMs, it's still advisable to avoid sharing sensitive information in direct messages without proper verification of the recipient's public key.
