...
:::info
Power limits will generally be lifted in the software if `is_licensed` is set to `true`. See [HAM Mode](../faq#amateur-radio-ham) for more information.
:::
...
Custom settings can be applied by using [supported software](../software).
