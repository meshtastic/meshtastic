...
A sender that hears no confirmation retransmits. A broadcast is sent up to three times in total and a direct message up to five. The interval between attempts is calculated from the packet's airtime and how much radio traffic is already on the air, rather than being a fixed value.
