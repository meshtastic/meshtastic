...
where possible, the design logo hyperlinks to the Meshtastic website, http://opensource.org/, or if that is not possible, the site includes a prominent link to the Meshtastic website at http://opensource.org/.
proper trademark symbols are used in connection with the Meshtastic Trademarks and the trademark attribution statement must appear as explained in Proper Trademark Use
the Logo Usage Guidelines are strictly observed
the site indicates clearly that it is not affiliated with or endorsed by Meshtastic; in addition, where possible:
the site must include the text "This site is not affiliated with or endorsed by the Open Source Initiative" prominently on any page that includes the Meshtastic Trademarks, and
...
To join the discussion about Meshtastic Trademark policies, and participate in shaping future policy visit the [Meshtastic Discourse](http://meshtastic.discourse.group).