...
It is highly recommended to download MQTT Explorer for analyzing the JSON threads that come across the broker. This can be downloaded from [MQTT Explorer’s website](http://mqtt-explorer.com/).
...
### Activity Card for Messaging

The activity card is useful to keep a record of incoming messages from the mesh. Below is an example of how the activity card would be set up.

![HA Activity Card](/img/software/mqtt/ha_activity_card.webp)
...
This same type of automation is very useful to trigger other actions in Home Assistant.  For example, you could turn on a fan when the temperature reaches a certain value, or play a sound on a speaker when a new message is received. 
