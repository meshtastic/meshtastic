...
- Connect your gateway node to Wi-Fi, by setting the `network.wifi_ssid`, `network.wifi_psk` and `network.wifi_enabled` preferences.
...
Any meshtastic node that has a direct connection to the internet (either via a helper app or installed Wi-Fi/4G/satellite hardware) can function as a "Gateway node".
