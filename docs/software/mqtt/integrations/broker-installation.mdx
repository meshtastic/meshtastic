...
If you get the above errors, try running `brew link mosquitto`. For further issues try `brew doctor` and follow the recommendations.
:::
...
```
:::note
If you are using the MacOS firewall, you will need to go into: `System Preferences > Security & Privacy > Firewall > Firewall Options` and add `mosquitto`.
:::
