...
The Python script _`interactiveSim.py`_ uses [MeshtasticD](/docs/meshtasticd/installation/) to simulate multiple instances of the device software. These instances communicate using TCP via the script, simulating the LoRa chip. The simulator forwards messages from the sender to all nodes within range, based on their simulated positions and the selected pathloss model (see [Pathloss Model](#pathloss-model)). **Note:** Packet collisions are not yet simulated.
...
The simulator runs the MeshtasticD application of Meshtastic firmware. You can use either [PlatformIO](https://meshtastic.org/docs/development/firmware/build) or [Docker](https://meshtastic.org/docs/software/linux/usage/#usage-with-docker) to run the firmware:
