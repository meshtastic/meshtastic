...
As of 1.3.48, there is a change in what is a valid json envelope for information sent by MQTT to a device for broadcast onto the mesh.
In addition to a "sender" and "payload" field, there must now be a "type":"sendtext" field for a valid envelope check.
'''
{
