...
sidebar_position: 4
desciption: Meshtastic UI (MUI) is a standalone interface for supported Meshtastic devices, enabling direct interaction without requiring a phone for most functions. It runs alongside the firmware, providing messaging, mapping, and configuration tools.
---
...
The map screen displays vector map tiles in PNG format, which must be stored on the SD card of the device. No map tiles are included, and must be manually added to the SD card in either a `/map` if using a singly style or `/maps/{STYLE}/` directory  if using multiple styles. 
