...
|------|-------|-------------|
| ![MQTT connected](/img/apple/mqttConnected.webp) | Connected | MQTT bridge is active — uplink and downlink both enabled. |
| ![MQTT uplink only](/img/apple/mqttUplinkOnly.webp) | Uplink Only | Publishing mesh packets to the broker but not subscribing to incoming packets. |
| ![MQTT disconnected](/img/apple/mqttDisconnected.webp) | Disconnected | MQTT is configured but not currently connected to the broker. |
