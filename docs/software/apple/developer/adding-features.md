...
No Xcode project changes are needed — `Meshtastic/Views/` is a synchronized folder, so the new file is picked up automatically. See [Project Generation](./codebase.md.md#project-generation) if you're touching `project.yml` itself (e.g. adding a new target).
