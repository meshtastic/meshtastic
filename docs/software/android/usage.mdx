...
You will need a device with Meshtastic installed to go any further. See the [getting started](../../getting-started/index.mdx) section for information on how to do this.
...
In order to start communicating with your mesh, you must select a region. This setting controls which frequency range your device uses and should be set according to your location. See [Region Settings](https://meshtastic.org../../getting-started/initial-config#set-regional-settings) for a list of region codes and their meanings.
...
- Tap on a node from the list to start Direct Messaging, request a position update, request a [traceroute](../../configuration/module/traceroute) or add it to your [Ignore Incoming Array](../../configuration/radio/lora#ignore-incoming-array).
- If you have an [Admin Channel](../../configuration/remote-admin) enabled on your devices, tapping on the node will also display an option to remotely configure the node.
...
- See [Device Config](../../configuration/radio/index.mdx) for radio settings.
- See [Module Config](../../configuration/module/index.mdx) for module settings.
- At the end of this list are buttons for Reboot, Shutdown, Factory reset, and NodeDB reset.

### Export rangetest.csv

Allows you to save all your network's position data with GPS coordinates into a .csv (comma separated value) file on your phone. This file can be imported into the spreadsheet application of your choice for easy viewing. This feature is similar but independent from the device [range test module](../../configuration/module/range-test), and results may differ.
