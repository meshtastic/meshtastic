...
Ignore the Anonynmous Usage check box and the "Report Bug" button at the bottom, they are not enabled and you cannot select them.  They are there for decoration purposes only.

[![Changing device name](/img/android/android-settings-mike-sm.webp)](/img/android/android-settings-mike.webp)

### Delete a radio

Deleting a listed, Bluetooth connected radio, is not handled by the app.  You will need to go into your phones Bluetooth settings and choose "Forget" the connection.  Only then will it be removed from the radio list.
