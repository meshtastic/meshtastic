...
Starting with firmware version 2.5.0, which introduces Public Key Cryptography (PKC) for Direct Messages (DMs) and also incorporates the use of Session IDs for Admin Messages, the security architecture of Meshtastic has reached a new level of robustness.

Below is a detailed overview of how Pre-Shared Key (PSK), PKC, and Session IDs are integrated into your system to secure Channels, DMs, and Admin Messages.
