...
3. Make the desired changes to the chosen variant.
4. Build the firmware by running: `pio run -e <variant_name>`
5. Once complete, download `<variant_name>.bin` or `<variant_name>.uf2` from `/workspace/firmware/.pio/build/<variant_name>.`
6. As flashing directly to the device isn't possible using Gitpod, upload using either:
