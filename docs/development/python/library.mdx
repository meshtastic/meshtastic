...
Below is a Python 3 example using the [Meshtastic Python API](https://python.meshtastic.org/)￼ to send a message over the mesh and to read/update a radio configuration setting. Before running the code, install the library by following the [installation instructions](/docs/software/python/cli/installation/).
...
Another example using Python 3 code to send a message to the mesh when Wi-Fi is enabled:
