...
An example using Python 3 code with the ["meshtastic" Python API package](https://python.meshtastic.org) to send a message to the mesh, get and set a radio configuration preference.  The ["meshtastic" Python API package](https://python.meshtastic.org) is typically installed using the command `pip3 install meshtastic`:
