...
3. Put the device into [bootloader mode](/docs/getting-started/flashing-firmware/nrf52/entering-bootloader-mode/). On most devices, double-press the reset button.
...
:::warning
The erase file must match your SoftDevice version: the wrong one can overwrite part of the SoftDevice and leave the device needing a hardware programmer to recover. If you copied `meshtastic_factory_erase.uf2` and nothing happened, your bootloader does not support it yet: use the preceding steps, or [update the bootloader](/docs/getting-started/flashing-firmware/nrf52/update-nrf52-bootloader/) first and check `INFO_UF2.TXT` again.
:::
