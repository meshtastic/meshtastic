...
:::info
The Meshtastic Android app can perform OTA updates itself — connect to your device over Bluetooth, then go to **Advanced → Firmware Update** and select a release. No third-party DFU app is required. The instructions below use a third-party app instead, which can help when the in-app update fails or for troubleshooting.
:::
