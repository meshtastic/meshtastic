...
Connect to your device over WiFi, either using the [software access point](/docs/1.2-End-of-life/settings/wifi#software-access-point) or [WiFi Client](/docs/1.2-End-of-life/settings/wifi#wifi-client). Then navigate to `meshtastic.local` (or your IP address). Your file will be available for download under `Extensions > File Browser > rangetest.csv` once it has been created by receiving messages.
