...
:::info Firmware Flash Notice
To enter into Espressif Service Mode (needed for esptool to flash firmware) press `BOOT` and `RESET` button at the same time. See picture below to find the buttons.
:::
