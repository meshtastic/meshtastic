...
Now both devices should be able to talk over the `gpio` channel. Send a text message from one to the other verify. Also run `--nodes` to verify the second node shows up.
