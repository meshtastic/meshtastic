---
id: serial
title: Serial Module Configuration
slug: /settings/moduleconfig/serial
sidebar_label: Serial
---

import Tabs from "@theme/Tabs";
import TabItem from "@theme/TabItem";

The serial module config options are: Enabled, Echo, Mode, Receive GPIO, Transmit GPIO, Baud Rate, Timeout, and Override Console Serial Port. Serial Module config uses an admin message sending a `ConfigModule.Serial` protobuf.

This is a simple interface to send messages over the mesh network by sending strings over a serial port. Anything you send the node will be turned into a message sent out over the mesh, and anything received from the mesh will be sent to the serial port. Note that this module does not (yet) allow arbitrary protobuf commands to be sent over the serial connection.

![image](https://user-images.githubusercontent.com/9000580/205529843-962c3187-8411-452c-b729-42c58b1571f5.png)

<object data="https://www.youtube.com/embed/HdOiGKBtapw?autohide=1&autoplay=0" width="100%" height="400"></object>

## Serial Module Config Values

### Enabled

Enables the serial module.

### Echo

If set, any packets you send will be echoed back to your device.

### Mode

Defaults to 'Simple'.

Available Values:

- `DEFAULT`
- `SIMPLE` operate as an dumb UART tunnel. What goes in will come out, Requires a channel named 'serial'.
- `PROTO` Exposes the Protobuf Client API on this serial port. You can use this to connect from another device. [API Reference](/docs/development/device/client-api)
- `TEXTMSG` Will send the string received over the serial port as a Text Message for Display on the other devices.
- `NMEA` Will output a NMEA 0183 Data stream containing the internal GPS or fixed position and other node locations as Waypoints (WPL).
- `CALTOPO` Will output NMEA 0183 Waypoints (WPL) every 10 seconds for all valid node locations, to be consumed by [CalTopo / SARTopo](/docs/software/integrations/caltopo.mdx).

### Receive GPIO Pin

Set the GPIO pin to the RXD pin you have set up.

### Transmit GPIO Pin

Set the GPIO pin to the TXD pin you have set up.

:::tip
Connect the TX pin to the other device's RX pin, and vice versa. Connect their grounds to each other (not necessary if they're both plugged into the same USB power source.)
:::

### Baud Rate

The serial baud rate.

### Timeout

The amount of time to wait before we consider your packet as "done".

### Override Console Serial Port 

If set to true, this will allow Serial Module to control (set baud rate) and use the primary USB serial bus for output. This is only useful for NMEA and CalTopo modes and may behave strangely or not work at all in other modes. Setting TX/RX pins in the Serial Module config will cause this setting to be ignored.

:::tip
Once module settings are changed, a **reset** is required for them to take effect.
:::

## Serial Module Config Client Availability

<Tabs
groupId="settings"
defaultValue="apple"
values={[
{label: 'Android', value: 'android'},
{label: 'Apple', value: 'apple'},
{label: 'CLI', value: 'cli'},
{label: 'Web', value: 'web'},
]}>
<TabItem value="android">

:::info

Serial Module Config options are available for Android.

1. Open the Meshtastic App
2. Navigate to: **Vertical Ellipsis (3 dots top right) > Radio Configuration > Serial**

:::

</TabItem>
<TabItem value="apple">

:::info
All serial module config options are available on iOS, iPadOS and macOS at Settings > Modules > Serial.
:::

</TabItem>
<TabItem value="cli">

:::info

All serial module config options are available in the python CLI. Example commands are below:

:::

|    Setting     |                                                                                              Acceptable Values                                                                                               |               Default                |
| :------------: | :----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------: | :----------------------------------: |
| serial.enabled |                                                                                               `true`, `false`                                                                                                |               `false`                |
|  serial.echo   |                                                                                               `true`, `false`                                                                                                |               `false`                |
|  serial.mode   |                                                                                 `DEFAULT` `SIMPLE` `PROTO` `TEXTMSG`, `NMEA`, `CALTOPO`                                                                      |              `DEFAULT`               |
|   serial.rxd   |                                                                                             GPIO Pin Number 1-39                                                                                             |       Default of `0` is Unset        |
|   serial.txd   |                                                                                             GPIO Pin Number 1-33                                                                                             |       Default of `0` is Unset        |
|  serial.baud   | `BAUD_DEFAULT` `BAUD_110` `BAUD_300` `BAUD_600` `BAUD_1200` `BAUD_2400` `BAUD_4800` `BAUD_9600` `BAUD_19200` `BAUD_38400` `BAUD_57600` `BAUD_115200` `BAUD_230400` `BAUD_460800` `BAUD_576000` `BAUD_921600` |            `BAUD_DEFAULT`            |
| serial.timeout |                                                                                          `integer` (milli seconds)                                                                                           | Default of `0` corresponds to 250 ms |
| serial.override_console_serial_port |                                                                                               `true`, `false`                                                                                                |               `false`                |


:::tip

Because the device will reboot after each command is sent via CLI, it is recommended when setting multiple values in a config section that commands be chained together as one.

```shell title="Example:"
meshtastic --set serial.enabled true --set serial.echo true
```

:::

```shell title="Enable / Disable Module"
meshtastic --set serial.enabled true
meshtastic --set serial.enabled false
```

```shell title="Enable / Disable Echo"
meshtastic --set serial.echo true
meshtastic --set serial.echo false
```

```shell title="Set Mode"
meshtastic --set serial.mode DEFAULT
meshtastic --set serial.mode PROTO
```

```shell title="Set RXD to GPIO pin number 7"
meshtastic --set serial.rxd 7
```

```shell title="Set TXD to GPIO pin number 28"
meshtastic --set serial.txd 28
```

```shell title="Set Baud Rate"
meshtastic --set serial.baud BAUD_DEFAULT
meshtastic --set serial.baud BAUD_576000
```

```shell title="Set Timeout to 15 milli seconds"
meshtastic --set serial.timeout 15
```

</TabItem>
<TabItem value="web">

:::info
All serial module config options are available in the Web UI.
:::

</TabItem>
</Tabs>

:::warning
GPIO access is fundamentally dangerous because invalid options can physically damage or destroy your hardware. Ensure that you fully understand the schematic for your particular device before trying this as we do not offer a warranty. Use at your own risk.

This module requires attaching a peripheral accessory to your device. It will not work without one.
:::

## Examples

Default is to use RX GPIO 16 and TX GPIO 17.

### Basic Usage

1. Enable the module by setting `serial.enabled` to `1`.
2. Set the pins (`serial.rxd` / `serial.txd`) for your preferred RX and TX GPIO pins. On tbeam boards it is recommended to use:
   - RXD 13
   - TXD 14
3. Set `serial.timeout` to the amount of time to wait before we consider your packet as "done".
4. (Optional) set serial.mode to TEXTMSG if you want to send messages to/from the general text message channel
5. Connect to your device over the serial interface at `38400 8N1`.

   With [tio](https://github.com/tio/tio) &ndash; `tio -e -b 38400 -f none /dev/myserialport`

6. Send a packet up to 237 bytes in length. This will get relayed over the mesh network.
7. (Optional) Set `serial.echo` to `1` and any message you send out will be echoed back to your device.

### Interfacing PIR Sensor With External Microcontroller

The following are examples of using either a Raspberry Pi Pico or Arduino Mini Pro connected to a PIR sensor to detect motion. When motion is detected, a message is sent via serial to the T-Beam. The T-Beam transmits the message as text over the default channel by utilizing the serial module in TXTMSG mode.

#### Meshtastic Software Configuration

- Serial module enabled, mode: TXTMSG
- GPIO Pins (For T-Beam) RX 13, TX 14
- 38400 Baud

#### Rasberry Pi Pico BOM

- A Raspberry Pi Pico running [CircuitPython](https://learn.adafruit.com/getting-started-with-raspberry-pi-pico-circuitpython)
- T-Beam V1.1 running Meshtastic
- PIR Sensor ([Adafruit Breadboard Model](https://www.adafruit.com/product/4871))

#### Raspberry Pi Pico Wiring

![image](/img/modules/Serial/pico-pir-wiring.png)

- TX pin 14 on the T-Beam to RX pin 2 on the Pico
- RX pin 13 on the T-Beam to TX pin 1 on the Pico
- PIR sensor Vcc and GND pins to Vcc/GND on breadboard respectively
- PIR sensor trigger line to Pico GPIO28 (Pico pin 34)
- GND pin on T-Beam to GND pin 38 on the Pico
- GND pin 38 on the Pico to breadboard ground rail
- 3V3 pin 36 on the Pico to the breadboard positive voltage rail
- Optional, to power the Pico off of the T-Beam instead of USB:
  - Connect 5V pin on T-Beam to Vbus pin 40 on the Pico

#### Circuit Python Code

```python
import time
import board
import busio
import digitalio

# Setup PIR sensor on GP28
pir = digitalio.DigitalInOut(board.GP28)
pir.direction = digitalio.Direction.INPUT

# Setup serial UART connection TX/RX on (GP0/GP1)
uart = busio.UART(board.GP0, board.GP1, baudrate=38400, timeout=0)

while True:
    if(pir.value == True):
        uart.write(bytes("Motion Detected", "ascii"))
        time.sleep(30)
    time.sleep(0.5)
```

#### Arduino Mini Pro BOM
- An Arduino Mini Pro with example sketch from below uploaded to it.
- T-Beam V1.1 running Meshtastic
- PIR Sensor ([Adafruit Breadboard Model](https://www.adafruit.com/product/4871))

#### Arduino Mini Pro Wiring

![image](/img/modules/Serial/arduino-mini-pro-pir-wiring.png)

- T-BEAM RX PIN 13 to TX PIN on the ARDUINO MINI
- T-BEAM TX PIN 14 to RX PINon the ARDUINO MINI
- T-BEAM PIN 3.3V to 3.3V PIN on the ARDUINO MINI
- T-BEAM PIN GND to GND PIN on the ARDUINO MINI
- ARDUINO MINI PIN 2 to OUT PIN on the PIR SENSOR
- ARDUINO MINI PIN 3.3V to 3.3V on the PIR SENSOR
- ARDUINO MINI PIN GND to GND PIN on the PIR SENSOR

#### Arduino Mini Pro PIR Code

```cpp
int LED = 13; // the pin to which the LED is connected
int PIR = 2;  // the pin to which the sensor is connected
int previousState = LOW; // previous state of the sensor


void setup() {
pinMode(LED, OUTPUT); // initialize the LED as an output
pinMode(PIR, INPUT);  // initialize the sensor as an input
Serial.begin(9600);   // initialize serial communication
}

void loop(){
   int currentState = digitalRead(PIR); // read the current state of the sensor
  if (currentState != previousState) { // check if the state has changed
    if (currentState == HIGH) { // check if there is motion
      digitalWrite(LED, HIGH);  // turn the LED on
      Serial.println("Motion Detected");
}
    else {
      digitalWrite(LED, LOW);  // turn the LED off
      Serial.println("No Motion");
    }
previousState = currentState; // update the previous state
  }
  delay(100); // small delay to avoid false sensor readings

}
```
### Interfacing with button an relay With External Microcontroller

Here are two different examples that work together or separately.
In the first code when the button is pressed the word "relay1toggle" will be sent via serial which will turn on or off the relay that is connected in the example below.

In the second code, the relay will be activated when t-beam receives the word "relay1toggle" or deactivated depending on its status.
If you want to activate or deactivate it through the meshtastic application you can also send a message to the channel or directly to the t-beam, with the word "on" or "off" and the relay will be activated or deactivated depending on its status.

Both situations are interesting, and can be explored also with nodered, and telegram use your imagination.

Thanks to Phil Manofsky for sharing this code with us.

#### Arduino Mini  Pro BOM


- An Arduino Mini Pro with example sketch from below uploaded to it.
- T-Beam V1.1 running Meshtastic
- Relay ([Adafruit Breadboard Model](https://www.adafruit.com/product/3191)
- Button ([Adafruit Breadboard Model](https://www.adafruit.com/product/1119)


#### Arduino Mini Pro / Button Wiring

![image](/img/modules/Serial/button.png)

- T-BEAM RX PIN 13 to TX PIN on the ARDUINO MINI
- T-BEAM TX PIN 14 to RX PINon the ARDUINO MINI
- T-BEAM PIN 3.3V to 3.3V PIN on the ARDUINO MINI
- T-BEAM PIN GND to GND PIN on the ARDUINO MINI
- ARDUINO MINI PIN 2 to  the BUTTON
- ARDUINO MINI PIN GND to the BUTTON

#### Arduino Mini Pro /Button Code

```cpp
const int buttonPin = 2;
String relayToggleCode = "relay1toggle";
int lastButtonState;
int buttonState;
int debounceTime = 50;


void setup() {

  Serial.begin(9600);
  pinMode(buttonPin, INPUT_PULLUP);
  lastButtonState = digitalRead(buttonPin);

}

void loop() {

  buttonState = digitalRead(buttonPin); //every loop, check the button state

  if (buttonState != lastButtonState) { //if a button state change is detected...

    lastButtonState = buttonState; //update the lastButtonState variable then...

    if (buttonState == LOW) { //check to see if the button is pressed
      delay(debounceTime); //wait a very short delay to make sure the change detected wasn't just noise
      buttonState = digitalRead(buttonPin); //read the button pin again

      if (buttonState == LOW) { // if the button pin is still LOW after debounce delay, that means the detected change was a real press, so we send the relay toggle command. If not, then we just assume the state change was noise and do nothing.

        sendRelayToggleCommand();

      }

    }

  }

}

void sendRelayToggleCommand() {

  Serial.println(relayToggleCode);

}

```

#### Arduino Mini Pro / Relay Wiring

![image](/img/modules/Serial/relay.png)

- T-BEAM RX PIN 13 to TX PIN on the ARDUINO MINI
- T-BEAM TX PIN 14 to RX PINon the ARDUINO MINI
- T-BEAM PIN 3.3V to 3.3V PIN on the ARDUINO MINI
- T-BEAM PIN GND to GND PIN on the ARDUINO MINI
- ARDUINO MINI PIN 2 to SIGNAL PIN on the RELAY
- ARDUINO MINI PIN GND to the GND on the RELAY
- ARDUINO MINI PIN VCC to the VCC on the RELAY

#### Arduino Mini Pro /relay Code

```cpp
//define sketch constants (pins, baud rate and other constants)
#define RELAY_PIN 2
#define BAUD_RATE 9600

int currentRelayState; //this variable can be checked to see the state of the relay pin and must be manually updated any time you change the state of the relay pin with digitalWrite(). HIGH means active coil, LOW means inactive coil

//these "code" variables are the commands to control the relay, they can be whatever you want them to be, but make sure they are not too simple like "on" or "off" or you will accidentally activate/deactivate your relay while sending basic text conversation over the mesh. For example, if the relayOffCode is just "off" and you send "Hey Steve, what time are you off work?", you've just turned off your relay accidentally because the string "off" was in your message.
const char* relayToggleCode = "relay1toggle";
const char* relayOnCode = "relay1on";
const char* relayOffCode = "relay1off";

// the MAX_INPUT variable defines the size of the incoming message buffer that checkStringForPasscode() examines to see if it contains any of the "codes" above, this value can be whatever you want but smaller numbers will get processed faster. As long as you send the right command within the first 30 bytes (characters) of a message from another node, your command will be seen by the Arduino and processed. (it's actually more like the first 24 bytes because you have to account for the first 6 characters that the Serial Module prepends to each incoming message in the form of the sending node's shortname (4 characters) which is always followed by ": " (another 2 characters, so 6 in total). 
const unsigned int MAX_INPUT = 30;

void setup() {
  Serial.begin(BAUD_RATE); //enable UART so Tbeam and Arduino can talk to each other
  pinMode(RELAY_PIN, OUTPUT);
  
  //the lines below were written so that the relay always starts off in the inactive state on the Arduino bootup.  But if you had some way of writing variables to non-volatile memory like flash memory you could save the most recent state of the relay to flash and set the relay to that state every time the Arduino is powered on. Just something to consider. 
  digitalWrite(RELAY_PIN, LOW); 
  currentRelayState = LOW;
}

void loop() {
  //the main loop() in this sketch is very simple, it just checks to see if any new data is in serial buffer and then if there is, it sends the next available byte (character) to the processIncomingByte() function
  while (Serial.available() > 0) {
    processIncomingByte(Serial.read());
  }
}

//Credit for the following function goes to Nick Gammon 
//This function takes the incoming bytes from the serial buffer and stores them in a string or char array. Everytime the function is called it adds the new byte or character to the input_line array until the array has the MAX_INPUT number of elements or until a new-line character '\n' is received. Carriage returns '\r' are ignored. When the '\n\' character is received it is appended to the array and the whole array is passed to the checkStringForPasscode() function.
void processIncomingByte(const byte inByte) {
  static char input_line[MAX_INPUT];
  static unsigned int input_pos = 0;

  switch (inByte) {
    case '\n':
      input_line[input_pos] = 0;
      checkStringForPasscode(input_line);
      input_pos = 0;
      break;
    case '\r':
      break;
    default:
      if (input_pos < (MAX_INPUT - 1))
        input_line[input_pos++] = inByte;
      break;
  }
}

//this function does the exciting stuff that you can see with your eyes. It stores the incoming char array as a String object (yes with a capital S, not to be confused with a string variable), so that we can process the data with the .indexOf() function. The indexOf() function is used to find a, let's call it a "sub-string", within the String object. If it finds the sub-string, the function returns the index of that sub-string. If the sub-string does not exist within the String object, the function returns a -1. So as long as .indexOf() does NOT return a -1, we know that the sub-string exists in the String.
void checkStringForPasscode(const char* data) {
  
  //stores the incoming message as a String object
  String inputString = data;

  if (inputString.indexOf(relayToggleCode) != -1) { //if relayToggleCode exists in inputString, toggle the relay...
    
    //to toggle the relay, we first check "currentRelayState". If it is LOW, then we toggle the relay HIGH, and if it is HIGH, we set it back to LOW. We also respond to the mesh with a confirmationMessage or an acknowledgement that a command was received and the Arduino has reacted to that command appropriately. This way you can know for sure that your relay is working properly even if you are miles away.
    if (currentRelayState == LOW) {
      digitalWrite(RELAY_PIN, HIGH);
      currentRelayState = HIGH;
      Serial.println("Relay 1 has received the command and is now ON");
    } else {
      digitalWrite(RELAY_PIN, LOW);
      currentRelayState = LOW;
      Serial.println("Relay 1 has received the command and is now OFF");
    }
  
  } else if (inputString.indexOf(relayOnCode) != -1) { //if the message contains "relay1on"...
  
    digitalWrite(RELAY_PIN, HIGH); //turn on the relay
    currentRelayState = HIGH; //update the currentRelayState variable
    Serial.println("Relay 1 has received the command and is now ON"); //reply to the mesh with acknowledgement
  
  } else if (inputString.indexOf(relayOffCode) != -1) { //if the message contains "relay1off"...
    
    digitalWrite(RELAY_PIN, LOW); //turn off the relay
    currentRelayState = LOW; //update the currentRelayState variable
    Serial.println("Relay 1 has received the command and is now OFF"); // reply to the mesh with acknowledgement
  
  }
  
}
```