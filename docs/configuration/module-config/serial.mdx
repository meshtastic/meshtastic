...
In the first code when the button is pressed the word "relay1toogle" will be sent via serial which will turn on or off the relay that is connected in the example below.
...
#### Arduino Mini_Pro BOM


- An Arduino Mini Pro with example sketchs from below uploaded to it.
- T-Beam V1.1 running Meshtastic
...
const char* relayOnCode = "on";
const char* relayOffCode = "off";
...
void checkStringForPasscode(const char* data) {
  ...
  if (inputString.indexOf(relayToggleCode) != -1) {
    if (currentRelayState == LOW) {
      digitalWrite(RELAY_PIN, HIGH);
      currentRelayState = HIGH;
      const char* confirmationMessage = "Relay 1 has received the command and is now ON";
      Serial.println(confirmationMessage);
    } else {
      digitalWrite(RELAY_PIN, LOW);
      currentRelayState = LOW;
      const char* confirmationMessage = "Relay 1 has received the command and is now OFF";
      Serial.println(confirmationMessage);
    }
  } else if (inputString.indexOf(relayOnCode) != -1) {
    digitalWrite(RELAY_PIN, HIGH);
    currentRelayState = HIGH;
    const char* confirmationMessage = "Relay 1 has received the command and is now ON";
    Serial.println(confirmationMessage);
