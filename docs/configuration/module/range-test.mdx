...
Enables the range test module. **Both Sender and Receiver must have the module enabled. On both iOS and Android to receive any range test packets the module must be enabled, the device screen may continue to show range test packets from older firmware versions.**
