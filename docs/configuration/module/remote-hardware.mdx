...
While configuring this module may be available in clients, setting and reading GPIO's is currently only possible using the [Meshtastic Python CLI](../../software/python-cli/index.mdx)
...
You can get the latest python tool/library with `pip3 install --upgrade meshtastic` on Windows/Linux/OS-X. See the [python section](../../software/python-cli/installation.mdx) for more details.
...
4. Check the channel has been created and copy the long "Complete URL" that contains all the channels on that device:
   ...
   meshtastic --info
   ```
5. Connect the remote device via USB (or use the [remote admin](../../configuration/remote-admin) feature to reach it through the mesh)
...
You can programmatically do operations from your own python code by using the Meshtastic `RemoteHardwareClient` class. See the [Python API](../../software/python-cli/installation.mdx) documentation for more details.
