...
From firmware version 2.5 on, also the route back to the origin is recorded, as well as the Signal-to-Noise Ratio (SNR) of each link. Nodes with older firmware or that can't decrypt the traceroute will be shown as "Unknown" (represented by an ID equal to 4294967295 or 0xFFFFFFFF in hexadecimal) if all nodes in the route are using at least firmware version 2.3.12.
...
The first ID shown is the device you are connected to with the CLI. As you can see, this packet went through one other node to get to its destination and back.
...
After selecting a node in the list, click on the "route" button in the top-right to send a traceroute request to the selected node. The result will be shown in message page of the node.
