...
:::info
- S&F Routers will store messages for all channels it can decode
  - It is best that the S&F Router to have a non-default Primary Channel to ensure messages are retrievable by the Android/iOS clients
- Android and iOS clients will request S&F replays via channel in which NodeInfo is heard
- CLI python users can request S&F client replay for any channel
:::
