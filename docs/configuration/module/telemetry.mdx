...
|    SCD4X*    |          0x62          |                         CO2, temperature and humidity                         |
|    SCD30*    |          0x61          |                         CO2, temperature and humidity                         |
|    SFA30*    |          0x5D          |                     Formaldehyde, temperature and humidity                    |
...
Note that some of the sensors listed above require I2C speeds that are not compatible with a screen. ESP32 based modules can re-clock the I2C bus, while other modules such as NRF variants can't, making some of these sensors not available in such configurations (marked with `*` in the table). See the [limitations](#limitations) section below for more information.
...
### Environment / Air Quality Metrics

The environment and air quality metrics in the telemetry module support a variety of sensors, but limitations apply. Support for I2C sensors that provide fields that are not included in the list above can potentially be added to the main firmware provided there is a GPL licensed library for us to include to support it, and the library size is not prohibitive. Since data is stored in the `nodedb` of the device, the telemetry module supports a limited amount of fields.
...
Currently, the majority of the sensors supported in the firmware are I2C sensors. Many of these sensors have a limit in the I2C Clock speed they can support, and may not be compatible with a screen if the MCU can't reclock its I2C bus (since screens use higher clock speeds than these sensors support). Reclocking the I2C bus is currently only possible on ESP32-based configurations. Other variants, will not be compatible with these sensors if the variant is compiled with `HAS_SCREEN=1`. If your variant doesn't use a screen, the firmware can be compiled with `HAS_SCREEN=0` to support additional sensors.