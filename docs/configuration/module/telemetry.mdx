...
|   AS3935[^as3935]    |    0x01, 0x02, 0x03    |        Lightning strike count (1h) and estimated storm distance        |
|        BH1750        |          0x23          |                            Light intensity                             |
| DS18B20 (via DS2482) |       0x18–0x1F        |                  Temperature (up to 8 1-Wire probes)                   |
|        HM330X        |          0x40          |                      Concentration units by size                       |
|       MAX17048       |          0x36          |                    Battery voltage and charge level                    |
|       PCT2075        |          0x37          |                              Temperature                               |
|        SCD30         |          0x61          |                CO<sub>2</sub>, temperature and humidity                |
|        SCD4X         |          0x62          |                CO<sub>2</sub>, temperature and humidity                |
|        SEN5X         |          0x69          |   Particulate matter by size; further metrics vary by model [^sen5x]   |
|        SEN6X         |          0x6B          |   Particulate matter by size; further metrics vary by model [^sen6x]   |
|        SFA30         |          0x5D          |                 Formaldehyde, temperature and humidity                 |
|        SPA06         |       0x76, 0x77       |                    Barometric pressure, Temperature                    |
|       TSL2561        |          0x29          |                            Light intensity                             |

[^as3935]: Wiring the IRQ pin and defining `AS3935_IRQ` in the variant avoids polling the sensor over I2C every second.

[^sen5x]: All SEN5X models report particulate matter by size.

    | Model | Temperature and humidity | VOC index | NOx index |
    | :---: | :----------------------: | :-------: | :-------: |
    | SEN50 |         &mdash;          |  &mdash;  |  &mdash;  |
    | SEN54 |         &check;          |  &check;  |  &mdash;  |
    | SEN55 |         &check;          |  &check;  |  &check;  |

[^sen6x]: All SEN6X models report particulate matter by size.

    | Model  | Temperature and humidity | VOC and NOx index | CO<sub>2</sub> | Formaldehyde |
    | :----: | :----------------------: | :---------------: | :------------: | :----------: |
    | SEN62  |         &check;          |      &mdash;      |    &mdash;     |   &mdash;    |
    | SEN63C |         &check;          |      &mdash;      |    &check;     |   &mdash;    |
    | SEN65  |         &check;          |      &check;      |    &mdash;     |   &mdash;    |
    | SEN66  |         &check;          |      &check;      |    &check;     |   &mdash;    |
    | SEN68  |         &check;          |      &check;      |    &mdash;     |   &check;    |
    | SEN69C |         &check;          |      &check;      |    &check;     |   &check;    |
