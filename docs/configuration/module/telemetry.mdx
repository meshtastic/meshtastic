...
|       ADS1X15        | 0x48, 0x49, 0x4A, 0x4B |    ADC channel voltages (4 channels per device, 8 with two devices)    |
| AS3935 | 0x01, 0x02, 0x03 | Lightning strike count in the past hour and estimated storm distance in km (see [AS3935](#as3935)) |
| BH1750 | 0x23 | Light intensity |
| DS18B20 (via DS2482-800/DS2484) | 0x18–0x1F | Temperature (up to 8 1-Wire probes via DS2482-800, 1 via DS2484) |
|        HM330X        |          0x40          |                      Concentration units by size                       |
|       MAX17048       |          0x36          |                    Battery voltage and charge level                    |
|       PCT2075        |          0x37          |                              Temperature                               |
| SCD30 | 0x61 | CO2, temperature and humidity |
| SCD4X | 0x62 | CO2, temperature and humidity |
| SEN5X | 0x69 | Particulate matter by size; further metrics vary by model (see [SEN5X models](#sen5x-models)) |
| SEN6X | 0x6B | Particulate matter by size; further metrics vary by model (see [SEN6X models](#sen6x-models)) |
| SFA30 | 0x5D | Formaldehyde, temperature and humidity |
| SPA06 (Mesh-Tracker-X1 only) | 0x76, 0x77 | Barometric pressure, Temperature |
|       TSL2561        |          0x29          |                            Light intensity                             |

#### AS3935

Wire the interrupt (IRQ) pin and define `AS3935_IRQ` in the variant to avoid an I&sup2;C read every second.

#### SEN5X models

All SEN5X models report particulate matter by size. The **SEN50** is the base model, reporting particulate matter only. The **SEN54** adds temperature, humidity and VOC index. The **SEN55** adds temperature, humidity, VOC index and NOx index.

#### SEN6X models

All SEN6X models report particulate matter by size, temperature and humidity. The **SEN62** is the base model, reporting no further metrics. The **SEN63C** adds CO2. The **SEN65** adds VOC and NOx index. The **SEN66** adds VOC and NOx index and CO2. The **SEN68** adds VOC and NOx index and formaldehyde. The **SEN69C** adds VOC and NOx index, CO2 and formaldehyde.
