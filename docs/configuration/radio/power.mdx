...
Note that scheduled (periodic) super deep sleep — where the device sleeps for a configured duration, wakes to perform tasks, then sleeps again — is not currently implemented in the firmware. The `sds_secs` value only controls the sleep duration when the device enters deep sleep due to a critically low battery.
...
|            power.sds_secs            |        `integer` (seconds)         |     Default of `0` is one year      |
