...
The power config options are: Power Saving, Shutdown after losing power, ADC Multiplier Override, Wait Bluetooth Interval, Super Deep Sleep Interval, Light Sleep Interval, Minimum Wake Interval, and Device Battery INA2xx Address. Power config uses an admin message sending a `Config.Power` protobuf.
...
<div class="no-print">
  <details>
    ...
    <div>
      ...
      | heltec_v3 & heltec_wsl_v3 | 5.1205 |
      | heltec_v4 | 5.1205 |
      | heltec_wireless_paper | 2 |
      | heltec_wireless_tracker 1.0/1.1 | 5.1205 |
      | heltec_T114 | 4.916 |
      | m5stack_coreink | 5 |
      | nano-g1-explorer | 2 |
      | nano-g2-ultra | 2 |
...
:::warning Not currently functional as a scheduled timer
The `sds_secs` configuration field exists in the firmware and is configurable, but the power management state machine does **not** currently implement a timed transition to super deep sleep. The setting only controls the sleep duration when the device is forced into deep sleep due to a critically low battery event.

Scheduled (periodic) super deep sleep — where the device would sleep for a configured duration, wake to perform tasks, and sleep again — is **not implemented** in the current firmware. See [meshtastic/meshtastic#2361](https://github.com/meshtastic/meshtastic/issues/2361) for more context.
:::

Super deep sleep (SDS) is the deepest sleep mode available. In this state the CPU and LoRa radio are fully powered down.

`0` for default of one year (effectively disabled)

### Light Sleep Interval

:::info ESP32 Only — Limited to specific roles and configurations
This setting only applies to **ESP32-based boards** (has no effect on nRF52/RP2040 boards).

Light sleep is only active when **Power Saving mode is enabled** (`is_power_saving = true`) **or** the device role is **Router** or **Router Late**. It is explicitly disabled for **Tracker**, **TAK Tracker**, and **Sensor** roles (those roles manage their own sleep through their respective modules), and it will not activate when **WiFi is in use**.
:::
...
|            power.sds_secs            |        `integer` (seconds)         |   Default of `0` is one year — see warning above   |
