...
How often we should try to get GPS position (in seconds), or zero for the default of once every 2 minutes, or a very large value (maxint) to update only once at boot. The Router and Router Late roles default to once a day instead.

This interval controls how often the node acquires a GPS fix. It does not control how often the node transmits its position, which is set by the broadcast interval.
...
A fixed position node broadcasts its position no more often than once every 6 hours. Setting a shorter broadcast interval does not override this floor. The same floor applies to any node whose position hasn't changed since its last broadcast.
...
Default of `0` is 1 hour, or 12 hours for the Router and Router Late roles

If smart broadcast is off we should send our position this often.

The firmware scales this interval with the number of online nodes, so a node on a larger mesh broadcasts less often than the configured value. Publishing position on the default channel also raises the interval to at least 1 hour.
...
2. Navigate to: **Settings >  Position**
...
|          position.gps_update_interval          |                                                            `integer` (seconds)                                                            | Default `0` is 2 minutes (1 day for routers) |
...
| position.broadcast_smart_minimum_interval_secs |                                                            `integer` (seconds)                                                            | Default of `0` is 5 minutes  |
|        position.position_broadcast_secs        |                                                            `integer` (seconds)                                                            | Default of `0` is 1 hour (12 hours for routers) |
...
```shell title="Set Position Broadcast Interval (Default of 0 is 1 hour, or 12 hours for routers)"
meshtastic --set position.position_broadcast_secs 0
meshtastic --set position.position_broadcast_secs 7200
```

A shorter interval does not always take effect. Publishing position on the default channel raises the interval to at least 1 hour, and a fixed or unchanged position holds the 6 hour floor whatever value you set.
