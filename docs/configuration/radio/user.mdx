...
In all the names below, #### represents the last four digits of the MAC adress, and each emoji consumes four bytes of the string.
...
Defaults to `Meshtastic ####`. Can be changed to any four-byte value or a single emoji (since emojis consume four characters).

### Long Name

A personalized identifier for your device.

Defaults to the `ShortName`. This can be changed to any string, including emoji, up to 36 bytes.
...
The name assigned to the Bluetooth connection. Always set to `ShortName_####`
