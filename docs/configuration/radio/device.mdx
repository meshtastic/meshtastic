...
A common mobile setup could look like a single node on your person. In an environment with a high amount of traffic, it is recommended to use CLIENT_MUTE as this node doesn't need to participate in packet routing. In an environment with a low amount of traffic, using CLIENT can be used to aid in network packet routing.

Another mobile setup could look like a node permanently mounted in a vehicle (with an external antenna) as well as a node on your person. The vehicle mounted node can be set to CLIENT while the node on your person can be set to CLIENT_MUTE. This is so the vehicle node can handle the network packet routing as it is better suited with the higher externally mounted antenna.
