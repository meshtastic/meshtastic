...
:::warning
Setting Wi-Fi credentials, or performing any other local administration, over an untrusted Wi-Fi network exposes that traffic to the network. See [Local Administration Over Wi-Fi](/docs/development/reference/encryption-technical#local-administration-over-wifi) for the trust-boundary details.
:::
