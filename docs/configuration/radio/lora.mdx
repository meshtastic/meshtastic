...
Not every preset is legal in every band. From firmware 2.8, the device only accepts presets permitted in the selected [Region](#region), and clients filter the preset picker to that list. An illegal preset is clamped back to the region's default.
...
4. `MEDIUM_TURBO` (500kHz bandwidth, so it carries the same regional restriction as `SHORT_TURBO`.)

5. `MEDIUM_FAST`

6. `MEDIUM_SLOW`

7. `LONG_TURBO` (Performs similarly to `LONG_FAST`, but with 500kHz bandwidth.)
...
`EU_868` offers this list without the three Turbo presets, as 500kHz bandwidth is not permitted in the 869.4 - 869.65 MHz band.

#### Narrowband Presets

Firmware 2.8 adds three narrowband preset families for bands that cannot accommodate the presets above. Each family is exclusive to its regions and cannot be selected anywhere else:
...
See [Narrowband Presets](/docs/overview/radio-settings#narrowband-presets) for their data rates and link budgets.

:::info
`EU_868`, `EU_866` and `EU_N_868` share the same regulatory domain, so selecting a Lite or Narrow preset while the region is `EU_868` switches the region to `EU_866` or `EU_N_868` respectively. Selecting one of the general purpose presets switches it back.
:::

:::warning
The Tiny presets transmit in only 15.6 kHz of bandwidth. A TCXO with a tolerance of ±5 ppm or better is **required**.
:::
...
Please be aware that values < 62.5kHz may require a TCXO on some hardware devices. The `TINY_FAST` and `TINY_SLOW` presets use `16` (15.6 kHz) and require one with a tolerance of ±5 ppm or better.
