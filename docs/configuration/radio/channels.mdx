...
### Mute
If enabled, messages received over the channel will not invoke the devices buzzer if present. UI notifications such as `"New Message From []"` are also blocked, decluttering the display.

Set by `false` to default for all channels.

:::note

When alerts are active, notification's containing a bell character ignore all mute settings.
...
```shell title="Toggle mute on the PRIMARY channel"
meshtastic --ch-set mute true --ch-index 0
meshtastic --ch-set mute false --ch-index 0
```
