...
Enabling Managed Mode blocks client applications from accessing all radio configurations. Once enabled, radio configurations can only be accessed through Remote Admin messages (firmware version 2.5 and later) or the legacy Admin channel (firmware version 2.4 and earlier). However, this setting is not required for remote node administration.
