...
Enabling Managed Mode blocks client applications from accessing all radio configurations. Once enabled, radio configurations can only be accessed through Remote Admin control messages or the legacy Admin channel. However, this setting is not required for remote node administration.

Before enabling Managed Mode, verify that the Remote Admin control or legacy Admin channel is working properly to prevent being locked out.
