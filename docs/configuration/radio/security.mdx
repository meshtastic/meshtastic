...
Enabling Managed Mode blocks client applications from accessing all radio configurations. Once enabled, radio configurations can only be accessed through Remote Admin messages or the legacy Admin channel. However, this setting is not required for remote node administration.
