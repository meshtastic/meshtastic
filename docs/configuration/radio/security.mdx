...
Enabling Managed Mode blocks client applications from accessing all radio configurations. However, this setting is not required for remote node administration. Once enabled, radio configurations can only be accessed through Admin control messages or the legacy Admin channel.

Before enabling Managed Mode, verify that the Admin control or legacy Admin channel is working properly to prevent being locked out.
