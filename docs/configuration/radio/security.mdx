...
Enabling Managed Mode restricts access to all radio configurations via client applications. This is not required to be set in order to remotely administer the node. Radio configurations will only be accessible through the Admin control messages or the legacy Admin channel. Ensure the Admin control or legacy Admin channel is functioning properly before enabling this mode to avoid being locked out. 
