...
For a home setup with a node in the attic or on the roof, use the CLIENT role as the attic/roof node is used to receive/forward traffic due to it being better suited with a higher mounted antenna. It is NOT recommended to use the ROUTER or REPEATER as these roles are reserved for best positioned in strategic locations to maximize the network's overall coverage. Adding "Not Monitored" to the Long Name of this node can inform others that this node's purpose is to simply receive/forward traffic and isn't monitored. For the rest of the nodes in the home, use the CLIENT_MUTE role as these nodes do not need to participate in packet routing.
...
For a mobile setup with a single node in a vehicle, use the CLIENT role. This node should be mounted as high as possible.
