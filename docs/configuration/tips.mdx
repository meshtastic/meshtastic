...
A home setup is a node in an attic or on the roof as well as a node or nodes in the home. The attic/roof node is used to receive/forward traffic as it is better suited with a higher mounted antenna. In this case, use the CLIENT role. It is NOT recommended to use ROUTER or REPEATER as these roles are reserved for best positioned in strategic locations to maximize the network's overall coverage. Adding "Not Monitored" to the Long Name of this node can inform others that this node's purpose is to simply receive/forward traffic and isn't monitored.

For the rest of the nodes in the home, use the CLIENT_MUTE role as these nodes do not need to participate in packet routing.
