...
For a home setup with multiple nodes, use the CLIENT role for the node in/on the attic/roof and the CLIENT_MUTE role for the rest of the nodes as these do not need to participate in packet routing. The attic/roof node is used to receive/forward traffic due to it being better suited with a higher mounted antenna. It is NOT recommended to use the ROUTER or REPEATER as these roles are reserved for best positioned in strategic locations to maximize the network's overall coverage. Adding "Not Monitored" to the Long Name of the attic/roof node can inform others that this node's purpose is to simply receive/forward traffic and isn't actively monitored.
...
For a mobile setup with a node permanently mounted in/on a vehicle (with an external antenna) as well as a node on your person, use the CLIENT role for the vehicle node and the CLIENT_MUTE for the node on your person. This is so the vehicle node can solely handle the network packet routing as it is better suited with the higher externally mounted antenna. For this reason, also use CLIENT_MUTE for all other nodes in the vehicle as well.
