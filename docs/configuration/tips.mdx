...
A home setup is a node in an attic or on the roof as well as a node or nodes in the home. The attic/roof node is used to receive/forward traffic as it is better suited with a higher mounted antenna. In this case, it is recommended to set this node's role type to CLIENT. It is NOT recommended to use ROUTER or REPEATER as these roles are reserved for best positioned in strategic locations to maximize the network's overall coverage. Adding "Not Monitored" to the Long Name of this node can inform others that this node's purpose is to simply receive/forward traffic and isn't monitored.

For the rest of the nodes in the home, it is recommended to use CLIENT_MUTE as these nodes do not need to participate in packet routing.

### Mobile setup

A mobile setup is one where you have a single node on your person or one or more in a vehicle. 

For a mobile setup with a single node on your person in an environment with a high amount of traffic, it is recommended to use CLIENT_MUTE as this node doesn't need to participate in packet routing. In an environment with a low amount of traffic, using CLIENT can be used to aid in network packet routing.

For a mobile setup with a node permanently mounted in/on a vehicle (with an external antenna) as well as a node on your person, the vehicle mounted node is recommended to be set to CLIENT while the node on your person set to CLIENT_MUTE. This is so the vehicle node can solely handle the network packet routing as it is better suited with the higher externally mounted antenna.
