...
For a mobile setup with a node permanently mounted in/on a vehicle (with an external antenna) as well as a node on your person, use the CLIENT role for the vehicle node and the CLIENT_MUTE role for the node on your person. This is so the vehicle node can solely handle the network packet routing as it is better suited with the higher externally mounted antenna. For this reason, also use CLIENT_MUTE role for all other nodes in the vehicle as well.
