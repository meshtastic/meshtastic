...
For a home deployment with a single node, use the CLIENT role as it is your only node and should participate in packet routing. This node should be mounted as high as possible.
...
For a mobile deployment with a single node on your person or in a vehicle, use the CLIENT_MUTE role as it doesn't need to participate in packet routing.
...
3. Enable a SECONDARY channel named "LongFast" with PSK "AQ==". For Apple devices, leave the channel name  blank.
