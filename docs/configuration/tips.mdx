...
The following deployment examples show which roles should be used and why.

#### Home Deployment

A home deployment is one where you have a single node or multiple nodes including a node in an attic or on the roof as well as a node or nodes in the home.

For a home deployment with a single node, use the CLIENT role. This node should be mounted as high as possible.

For a home deployment with multiple nodes, use the CLIENT role for the node in/on the attic/roof and the CLIENT_MUTE role for the rest of the nodes as these do not need to participate in packet routing. The attic/roof node is used to receive/forward traffic due to it being better suited with a higher mounted antenna. It is NOT recommended to use the ROUTER or REPEATER roles as they are reserved for nodes best positioned in strategic locations to maximize the network's overall coverage. Adding "Not Monitored" to the Long Name of the attic/roof node can inform others that this node's purpose is to simply receive/forward traffic and isn't actively monitored.

#### Mobile Deployment

A mobile deployment is one where you have a single node on your person or one or more in a vehicle. 

For a mobile deployment with a single node on your person in an environment with a high amount of traffic, use the CLIENT_MUTE role as it doesn't need to participate in packet routing. In an environment with a low amount of traffic, use the CLIENT role as it can be used to aid in network packet routing.

For a mobile deployment with a single node in a vehicle, use the CLIENT_MUTE role. This node should be mounted as high as possible.

For a mobile deployment with a node permanently mounted in/on a vehicle (with an external antenna) as well as a node on your person, use the CLIENT role for the vehicle node and the CLIENT_MUTE role for the node on your person. This is so the vehicle node can solely handle the network packet routing as it is better suited with the higher externally mounted antenna. For this reason, also use the CLIENT_MUTE role for all other nodes in the vehicle as well.
