...
For a home setup with multiple nodes, use the CLIENT role for the node in/on the attic/roof and the CLIENT_MUTE role for the rest of the nodes as these nodes do not need to participate in packet routing. The attic/roof node is used to receive/forward traffic due to it being better suited with a higher mounted antenna. It is NOT recommended to use the ROUTER or REPEATER as these roles are reserved for best positioned in strategic locations to maximize the network's overall coverage. Adding "Not Monitored" to the Long Name of the attic/roof node can inform others that this node's purpose is to simply receive/forward traffic and isn't actively monitored.

### Mobile Setup

A mobile setup is one where you have a single node on your person or one or more in a vehicle. 

For a mobile setup with a single node on your person in an environment with a high amount of traffic, use the CLIENT_MUTE role as this node doesn't need to participate in packet routing. In an environment with a low amount of traffic, use the CLIENT role as it can be used to aid in network packet routing.
