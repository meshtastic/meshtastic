...
- `CLIENT` nodes receive, send, and intelligently repeat messages to help the mesh network.
  ...
  - Use when you're in remote areas (hiking, skiing, biking) where every node helps maintain connectivity.

- `CLIENT_MUTE` nodes can send and receive messages but never repeat them.
  - Use for your personal handheld when you're near a more powerful node (like your rooftop node) in a busy network.
  - Use when you have several nodes close together - set the best-positioned one to `CLIENT` and the others to `CLIENT_MUTE`.
