...
Default is `unset` which equates to `LongFast`. Presets are pre-defined modem settings (Bandwidth, Spread Factor, and Coding Rate) which influence both message speed and range. The default will provide a strong mixture of speed and range, for most users.
...
`ShortFast` (Fastest, highest bandwidth, lowest airtime, shortest range)
`ShortSlow`
`MediumFast`
`MediumSlow`
`LongFast` (Default)
`LongSlow`
`VeryLongSlow` (Slowest, lowest bandwidth, highest airtime, longest range)
