...
| `REPEATER`[^1]  |   Repeater - Mesh packets will be rebroadcast by this device without showing the device as part of the mesh network. This role is useful for a cleaner mesh setup where users do not need information about the Repeater node, or to prevent direct messaging to an unmonitored Repeater. Channel and modem settings must be identical to the mesh network being repeated. Please see Rebroadcast Mode for additional settings specific to this role.[^1]    |
