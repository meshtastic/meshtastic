...
If set, the device will attempt to save power by shutting off Bluetooth, Wifi, and any attached screen (if applicable). This setting is automatically set to True when the the device is in a standalone mode (Router mode, but not Router-Client mode) and will not need these resources anyway. This is especially useful when the device is powered from a low-current source, such as solar.
