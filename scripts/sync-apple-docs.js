...
async function main() {
  ...
  // ── Write Docusaurus category files ──────────────────────────────────────
  // These are not sourced from the Apple repo — they are generated by this
  // script so the sidebar section always exists after a sync.
  ...
  for (const { file, content } of CATEGORY_FILES) {
