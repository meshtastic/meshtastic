---
id: asennus
title: Meshtastic Python CLI asennus
...
Varmista ennen asennusta, että järjestelmäsi täyttää seuraavat vaatimukset:
...
- **Python**: Python 3 on asennettava järjestelmääsi. Tarkista `python3 -V` ja asenna se tarvittaessa.
- **pip**: Python pakettiasentaja pip tulee asentaa. Tarkista `pip -V` ja asenna se tarvittaessa.
...
- Tarkista, että tietokoneellesi on asennettu tarvittavat sarja-ajurit

  - Yhdistä Meshtastic laitteesi USB-porttiin
  - Käytä komentoa
    ```shell
    lsusb
    ```
  - Sinun pitäisi nähdä jotain tälläistä:
...
- Tarkista, että tietokoneessasi on Python 3 asennettu.

  - Käytä komentoa
    ```shell
    python3 -V
    ```
  - Jos tämä ei palauta versiota, asenna python
    ```shell
    sudo apt-get update
    sudo apt-get install python3
    ```

- Pip on yleensä asennettu, jos käytät python 3 versiota >= 3.4

  - Tarkista tämän komennon avulla, että pip on asennettu
    ```shell
    pip3 -V
    ```
  - Jos tämä ei näytä versiota, asenna pip
    ```shell
...
- Tarkista, että tietokoneellesi on asennettu tarvittavat sarja-ajurit
  - Yhdistä Meshtastic laitteesi USB-porttiin
  - Navigate to `Apple Menu  > About This Mac > System Report... > Laitteisto > USB`
...
- Tarkista, että tietokoneesi on asennettu Python 3.
  - Käytä komentoa
    ```shell
    python3 -V
    ```
  - If this does not return a version, install [python](https://www.python.org)
    ...
    - Tarkista tällä komennolla, että Homebrew on asennettu
      ```shell
      brew -v
      ```
...
- Pip on yleensä asennettu, jos käytät python 3 versiota >= 3.4
  ...
  - Tarkista tällä komennolla, että pip on asennettu
    ```shell
    pip3 -V
    ```
...
- Asenna meshtastic:
  ...
  ```
  (`[cli]` loppuliite asentaa muutamia valinnaisia riippuvuuksia, jotka vastaavat vanhempia CLI-versioita)
...
- Tarkista, että tietokoneellesi on asennettu tarvittavat sarja-ajurit
  - Yhdistä Meshtastic laitteesi USB-porttiin
  - Avaa Laitteiden Hallinta
  - `Porttien (COM & LPT)` alla sinun pitäisi nähdä jotain `Silicon LabsCP210X USB:stä UART Bridge (COM5)` tyylistä
    - If there is no serial device shown that matches the device you are using, please review our [Install Serial Drivers](/docs/getting-started/serial-drivers/) page before proceeding.
- Tarkista, että tietokoneessasi on Python 3 asennettu.
  - Käytä komentoa
    ```powershell
    py -V
    ```
  - Jos tämä ei palauta versiota, asenna [python](https://www.python.org)

:::info
Kun asennat Pythonin, varmista, että valitse vaihtoehto "Add Python to PATH" tai valitse laatikko, joka sanoo "Add Python to environment variables". If you missed this during installation, you can add Python to your system's PATH manually after installation. Failing to do so may result in errors when trying to use Python or pip commands.
:::
...
- Lataa Termux ja päivitä pakettilista
  ```shell
  pkg update
  ```
...
- Asenna python
  ...
  pkg install python
  ```
- Päivitä pip ja asennetut mestasticin ja muiden sen osien riippuvuudet
  ```shell
...
**Saatat joutua sulkemaan ja avaamaan CLI uudelleen. The path variables may or may not update for the current session when installing.**
