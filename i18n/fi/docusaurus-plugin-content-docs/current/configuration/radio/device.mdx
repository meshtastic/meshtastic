...
| CLIENT_MUTE    | Laite, joka ei välitä paketteja muilta laitteilta.                                                                                                                              | Situations where a device needs to participate in the network without assisting in packet routing, reducing network load.                                   |
| CLIENT_HIDDEN  | Laite, joka lähettää vain tarvittaessa tai virransäästotilassa.                                                                                                                 | Use in stealth/hidden deployments or to reduce airtime/power consumption while still participating in the network.                                          |
...
| TRACKER        | Lähettää GPS-sijaintitiedot ensisijaisesti.                                                                                                                                     | Tracking the location of individuals or assets, especially in scenarios where timely and efficient location updates are critical.                           |
| LOST_AND_FOUND | Lähettää laitteen sijainnin viestillä oletuskanavalle sen löytämisen helpottamiseksi.                                                                                           | Used for recovery efforts of a lost device.                                                                                                                 |
| SENSOR         | Lähettää telemetriatiedot ensisijaisesti.                                                                                                                                       | Deploying in scenarios where gathering environmental or other sensor data is crucial, with efficient power usage and frequent updates.                      |
| TAK            | Optimoitu ATAK-järjestelmän viestintään, joka vähentää tavanomaisia lähetyksiä.                                                                                                 | Integration with ATAK systems (via the Meshtastic ATAK Plugin) for communication in tactical or coordinated operations.                                     |
| TAK_TRACKER    | Ottaa käyttöön automaattisen TAK PLI -lähetyksen vähentäen tavanomaisia lähetyksiä.                                                                                             | Standalone PLI integration with ATAK systems for communication in tactical or coordinated operations.                                                       |
...
| -------------- | ----------------- | ---------------- | ----------------- | ---------------- | --------------------- | ----------------------- |
| CLIENT         | Yes               | Yes              | Regular           | Yes              | No                    | Kyllä                   |
| CLIENT_MUTE    | Kyllä             | Yes              | Lowest            | No               | No                    | Kyllä                   |
| CLIENT_HIDDEN  | Yes               | Yes              | Lowest            | Local Only       | No                    | No                      |
| CLIENT_BASE    | Yes               | Kyllä            | Regular           | Kyllä            | Favorites Only[^3]    | Yes                     |
| TRACKER        | Yes               | No               | Regular / Low     | Awake Only[^1]   | No                    | Yes                     |
| LOST_AND_FOUND | Yes               | No               | Regular           | Kyllä            | No                    | Yes                     |
| SENSOR         | Yes               | No               | Regular / Low     | Awake Only[^1]   | No                    | Yes                     |
| TAK            | Yes               | Optional         | Regular           | Yes              | No                    | Yes                     |
| TAK_TRACKER    | Kyllä             | Optional         | Regular           | Kyllä            | No                    | Yes                     |
| ROUTER         | No[^2]            | No               | High              | Yes              | Yes                   | Kyllä                   |
| ROUTER_LATE    | Yes               | Yes              | High              | Yes              | No                    | Kyllä                   |
| REPEATER       | Kyllä             | No               | High              | Kyllä            | Kyllä                 | No                      |
...
Poista käytöstä kolmoisnapautuksen GPS:n käyttöönoton painike.
...
| Setting                           | Acceptable Values                                                  | Oletus            |
| --------------------------------- | ------------------------------------------------------------------ | ----------------- |
