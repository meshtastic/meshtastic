...
| CLIENT_MUTE    | Olyan eszköz, amely nem továbbít más eszközöktől érkező csomagokat.                                                                                                                | Situations where a device needs to participate in the network without assisting in packet routing, reducing network load.                                   |
| CLIENT_HIDDEN  | Eszköz, amely csak szükség esetén sugároz, rejtettség vagy energiatakarékosság miatt.                                                                                              | Use in stealth/hidden deployments or to reduce airtime/power consumption while still participating in the network.                                          |
...
| TRACKER        | GPS-pozíció csomagok elsődleges sugárzása.                                                                                                                                         | Tracking the location of individuals or assets, especially in scenarios where timely and efficient location updates are critical.                           |
| LOST_AND_FOUND | Rendszeresen sugározza a helyzetet az alapértelmezett csatornára az eszköz visszakeresésének segítésére.                                                                           | Used for recovery efforts of a lost device.                                                                                                                 |
| SENSOR         | Telemetriai csomagok elsődleges sugárzása.                                                                                                                                         | Deploying in scenarios where gathering environmental or other sensor data is crucial, with efficient power usage and frequent updates.                      |
| TAK            | ATAK rendszerkommunikációra optimalizált, csökkenti a rutin-sugárzásokat.                                                                                                          | Integration with ATAK systems (via the Meshtastic ATAK Plugin) for communication in tactical or coordinated operations.                                     |
| TAK_TRACKER    | Automatikus TAK PLI sugárzást engedélyez és csökkenti a rutin-sugárzásokat.                                                                                                        | Standalone PLI integration with ATAK systems for communication in tactical or coordinated operations.                                                       |
...
| -------------- | ---------------- | --------------- | ----------------- | --------------- | -------------------- | ---------------------- |
| CLIENT         | Yes              | Yes             | Regular           | Yes             | No                   | Igen                   |
| CLIENT_MUTE    | Igen             | Yes             | Lowest            | No              | No                   | Igen                   |
| CLIENT_HIDDEN  | Yes              | Yes             | Lowest            | Local Only      | No                   | No                     |
| CLIENT_BASE    | Yes              | Igen            | Regular           | Igen            | Favorites Only[^3]   | Yes                    |
| TRACKER        | Yes              | No              | Regular / Low     | Awake Only[^1]  | No                   | Yes                    |
| LOST_AND_FOUND | Yes              | No              | Regular           | Igen            | No                   | Yes                    |
| SENSOR         | Yes              | No              | Regular / Low     | Awake Only[^1]  | No                   | Yes                    |
| TAK            | Yes              | Optional        | Regular           | Yes             | No                   | Yes                    |
| TAK_TRACKER    | Igen             | Optional        | Regular           | Igen            | No                   | Yes                    |
| ROUTER         | No[^2]           | No              | High              | Yes             | Yes                  | Igen                   |
| ROUTER_LATE    | Yes              | Yes             | High              | Yes             | No                   | Igen                   |
| REPEATER       | Igen             | No              | High              | Igen            | Igen                 | No                     |
...
Letiltja a GPS be/ki kapcsolásához szükséges hármas gombnyomást.
...
| Setting                           | Acceptable Values                                                  | Alapértelmezett           |
| --------------------------------- | ------------------------------------------------------------------ | ------------------------- |
