...
### Adafruit IO für Meshtastic

Adafruit IO kann verwendet werden, um über json/mqtt Telemetrie und Nachrichten eines Meshtastic Netzwerks grafisch darzustellen. Das folgende Beispielskript wird Nachrichten eines Knotens empfangen und Spannung, RSSI, SNR und Nachrichten in einzelnen Feeds auf Adafruit IO veröffentlichen. Falls ein Feed nicht existiert, wird er erstellt. Beachten Sie jedoch, dass das kostenlose Adafruit Konto auf 10 Feeds beschränkt ist. Sobald Ihre Feeds mit Daten empfangen, können Sie Dashboards erstellen, um Diagramme und Anzeigen mit den Daten darzustellen.

:::info

Um dieses Skript nutzen zu können, benötigen Sie ein Adafruit IO Konto ein funktionierender MQTT Broker Setup und ein Mesh Knoten, der an den MQTT Broker Daten sendet.

:::

Sie müssen die ini Beispieldatei mit Ihrem Adafruit IO und MQTT Anmeldedaten ändern. (Code Repository)[https://bitbucket.org/tavdog/mesh_aio_logger/src/main/]
...
USER = a
KEY = a
;lassen Sie FEED_GROUP als Standard wenn Sie keine separate benötigen.
FEED_GROUP = Default
[LOG]
VOLTAGE = true
MESSAGE = true
