...
sidebar_label: MQTT
description: Naučte se spravovat MQTT servery, šifrování a další pro vaše Meshtastic zařízení.
---
...
## Hodnoty konfigurace modulu MQTT
...
Server pro MQTT. Pokud není nastaveno, použije se výchozí veřejný server.
...
Pokud je povoleno, pokusíme se navázat bezpečné připojení pomocí TLS.

### Kořenové téma

Kořenové téma pro MQTT zprávy. This is useful if you want to use a single MQTT server for multiple meshtastic networks and separate them via ACLs.

### Proxy klient povolen

Pokud je povoleno, nechte zařízení používat síťové připojení klienta (např. telefon) pro připojení k MQTT serveru. If false, it uses the device's network connection which you have to enable via the [network settings](/docs/configuration/radio/network).
