...
| CLIENT_MUTE    | Zariadenie, ktoré nepreposiela pakety z ďalších zariadení.                                                                                                                   | Situations where a device needs to participate in the network without assisting in packet routing, reducing network load.                                   |
| CLIENT_HIDDEN  | Zariadenie, ktoré vysiela len podľa potreby pre utajenie, alebo úsporu energie.                                                                                              | Use in stealth/hidden deployments or to reduce airtime/power consumption while still participating in the network.                                          |
...
| TRACKER        | Prioritne vysiela pakety polohy GPS.                                                                                                                                         | Tracking the location of individuals or assets, especially in scenarios where timely and efficient location updates are critical.                           |
| LOST_AND_FOUND | Pravidelne vysiela polohu ako správu na predvolený kanál, aby pomohol pri obnove zariadenia.                                                                                 | Used for recovery efforts of a lost device.                                                                                                                 |
| SENSOR         | Prioritne vysiela telemetrické pakety.                                                                                                                                       | Deploying in scenarios where gathering environmental or other sensor data is crucial, with efficient power usage and frequent updates.                      |
| TAK            | Optimalizované pre systémovú komunikáciu ATAK, znižuje rutinné vysielanie.                                                                                                   | Integration with ATAK systems (via the Meshtastic ATAK Plugin) for communication in tactical or coordinated operations.                                     |
| TAK_TRACKER    | Umožňuje automatické vysielanie TAK PLI a znižuje rutinné vysielanie.                                                                                                        | Standalone PLI integration with ATAK systems for communication in tactical or coordinated operations.                                                       |
