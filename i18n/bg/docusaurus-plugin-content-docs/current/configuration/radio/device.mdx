...
| Device Role    | Описание                                                                                                                                                            | Best Uses                                                                                                                                                   |
| -------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------- |
...
| CLIENT_MUTE    | Устройство, което не препредава пакети от други устройства.                                                                                                         | Situations where a device needs to participate in the network without assisting in packet routing, reducing network load.                                   |
...
| TRACKER        | Излъчва приоритетно пакети за GPS позиция                                                                                                                           | Tracking the location of individuals or assets, especially in scenarios where timely and efficient location updates are critical.                           |
...
| SENSOR         | Излъчва приоритетно телеметрични пакети.                                                                                                                            | Deploying in scenarios where gathering environmental or other sensor data is crucial, with efficient power usage and frequent updates.                      |
...
| -------------- | --------------- | -------------- | ----------------- | -------------- | ------------------- | --------------------- |
| CLIENT         | Yes             | Yes            | Regular           | Yes            | No                  | Да                    |
| CLIENT_MUTE    | Да              | Yes            | Lowest            | No             | No                  | Да                    |
| CLIENT_HIDDEN  | Yes             | Yes            | Lowest            | Local Only     | No                  | No                    |
| CLIENT_BASE    | Yes             | Да             | Regular           | Да             | Favorites Only[^3]  | Yes                   |
| TRACKER        | Yes             | No             | Regular / Low     | Awake Only[^1] | No                  | Yes                   |
| LOST_AND_FOUND | Yes             | No             | Regular           | Да             | No                  | Yes                   |
| SENSOR         | Yes             | No             | Regular / Low     | Awake Only[^1] | No                  | Yes                   |
| TAK            | Yes             | Optional       | Regular           | Yes            | No                  | Yes                   |
| TAK_TRACKER    | Да              | Optional       | Regular           | Да             | No                  | Yes                   |
| ROUTER         | No[^2]          | No             | High              | Yes            | Yes                 | Да                    |
| ROUTER_LATE    | Yes             | Yes            | High              | Yes            | No                  | Да                    |
| REPEATER       | Да              | No             | High              | Да             | Да                  | No                    |
