...
: The low-level software programmed onto a Meshtastic device, controlling its hardware functions and enabling it to communicate within the mesh network using LoRa technology. Фърмуер
...
OTA
: Abbreviation for "Over the Air". Typically used to reffer to a OTA update which is one that happens over a wireless network (over the air). 
