...
XEdDSA packet signing is being added in firmware 2.8.X to further secure node identity claims and the authenticity of subsequent messages. It uses the XEdDSA system from Signal to take existing X25519 key pairs and produce ED25519 signatures from them, so nodes do not need to manage a second key pair. These signatures are 64 bytes long, which is a significant cost given the 256 byte LoRa packet limit.
