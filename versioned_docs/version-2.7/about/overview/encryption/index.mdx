...
Meshtastic provides AES256-CTR encryption for the payload of each packet when sending via LoRa, with a different key for each [channel](/docs/2.7/configuration/radio/channels/). The [packet header](/docs/2.7/overview/mesh-algo/#layer-1-unreliable-zero-hop-messaging) is always sent unencrypted, which allows nodes to relay packets they can't decrypt as well. One can disable this by setting a different [rebroadcast mode](/docs/2.7/configuration/radio/device#rebroadcast-mode).
...
The device will decrypt the payload before sending it to a client app via BLE, serial, WiFi/Ethernet. For MQTT, you can [specify](/docs/2.7/configuration/module/mqtt#encryption-enabled) whether you want to send an encrypted or unencrypted payload.

## Is it as secure as WiFi WPA3, HTTPS TLS1.3, or Signal?
