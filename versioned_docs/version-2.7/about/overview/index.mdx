...
8. **Payload (Encrypted)** - max 239 bytes  
   ...
   - **Want Response** - 1 bit  
   Determines if the message is requesting a response from the recipient
...
In addition to the core routing fields described above, this example shows several additional metadata fields which are added to the MeshPacket by the receiving node:
