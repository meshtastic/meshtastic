...
- [Meshtastic Web API](/docs/2.7/community/software/meshtastic-web-api) - Meshtastic Web API provides a RESTful interface to interact with a Meshtastic node via a serial connection.
- [Mesh-metrics](/docs/2.7/community/software/community-mesh-metrics) - Meshtastic repeater metrics scraper using meshtastic-cli to be ingested by node_exporter for use in Prometheus or Victoria Metrics Time series DB with a Grafana dashboard and sample alerting rules.
- [Node-RED Messages Node](/docs/2.7/community/software/community-node-red-messages) - Node-RED node to send and receive packets / text messages from a device connected via HTTP.
- [Harbor Scale - Grafana](/docs/2.7/community/software/harbor-scale) - A cloud-based solution for collecting, storing, and visualizing Meshtastic telemetry data using grafana.
- [MeshApp](https://github.com/smikme/meshapp) - A GPL-3.0 cross-platform desktop client for Meshtastic on Windows, macOS, and Linux. Connects over TCP, Serial / USB, or BLE and combines multi-channel chat, direct messages, device configuration, telemetry, node maps, traceroute history, LoRa packet inspection, MQTT proxy support, and a built-in Lua automation IDE. 
