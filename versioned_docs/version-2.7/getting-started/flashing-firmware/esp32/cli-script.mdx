...
:::note
The [T-Beam 0.7](/docs/2.7/hardware/devices/lilygo/tbeam/?t-beam=0.7) board is an earlier version of the T-Beam board, and due to changes in the design in subsequent iterations this board uses a specific firmware file different from the other T-Beam boards.

`firmware-tbeam0.7-X.X.X.xxxxxxx.bin` is the correct firmware. `firmware-tbeam-X.X.X.xxxxxxx.bin` is incompatible. For all other [T-Beam](/docs/2.7/hardware/devices/lilygo/tbeam) boards review the resources section for the correct selection.
:::
...
Use the `--web` option to include the embedded [Web Client](/docs/2.7/software/web-client/) during installation of the firmware.
