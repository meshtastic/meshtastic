...
Additionally, while configuring this module may be available in clients, setting and reading GPIO's is currently only possible using the [Meshtastic Python CLI](/docs/2.7/software/python/cli)
...
2. Navigate to: **Settings >  Remote Hardware**
   :::
...
- Receive a notification over the mesh if any GPIO changes state.
  Note that it cannot detect fast changes like button presses. For this, look at the [Detection Sensor module](/docs/2.7/configuration/module/detection-sensor) instead.
...
You can get the latest python tool/library with `pip3 install --upgrade meshtastic` on Windows/Linux/OS-X. See the [python section](/docs/2.7/software/python/cli/installation) for more details.
...
4. Check the channel has been created and copy the long "Complete URL" that contains all the channels on that device:
   ...
   meshtastic --info
   ```
5. Connect the remote device via USB (or use the [remote admin](/docs/2.7/configuration/remote-admin) feature to reach it through the mesh)
...
You can programmatically do operations from your own python code by using the Meshtastic `RemoteHardwareClient` class. See the [Python API](/docs/2.7/software/python/cli/installation) documentation for more details.
