...
import QRCode from "@site/versioned_docs/version-2.7/blocks/_qr-code.mdx";

## Roles

It is strongly recommended to keep your [ROLE](/docs/2.7/configuration/radio/device#roles) set to `CLIENT`, `CLIENT_MUTE`, or `CLIENT_BASE`. Only use other roles if you have a specific, well-understood reason to do so. Read our [blog post about choosing the best role](/blog/choosing-the-right-device-role/).
...
These roles are for very specific applications. Before changing from `CLIENT`, carefully [review the documentation](/docs/2.7/configuration/radio/device#roles) to understand the implications. Additionally, coordinating this with your local community to ensure its impact is a positive one rather than a negative one.
...
Telemetry is shared over your [PRIMARY channel](/docs/2.7/configuration/radio/channels#role). This means that if your node has acquired GPS coordinates from an integrated GPS chip, or from your mobile device, your coordinates will be sent to the mesh over this channel, using its defined encryption (if any).
...
For detailed information on position precision settings and how to configure them, please refer to the [Position Precision documentation](/docs/2.7/configuration/radio/channels/#position-precision).
...
Meshtastic nodes will rebroadcast all packets if they share [LoRa modem settings](/docs/2.7/configuration/radio/lora#lora-config-values), regardless of encryption (unless [Rebroadcast mode](/docs/2.7/configuration/radio/device#rebroadcast-mode) is set to `LOCAL_ONLY`).

:::info
If you would like your nodes to include/expand the "public" mesh, you must use the default [modem preset](/docs/2.7/configuration/radio/lora#modem-preset) `LONG_FAST`. If you change your PRIMARY channel name, you must manually set the LoRa channel to the default for your region (see above).
:::

### Hop Count

It is **strongly** recommended to leave your [MAX HOPS](/docs/2.7/configuration/radio/lora#max-hops) set to 3 unless you're sure you need more (or less) to reach your destination node. Unnecessarily high hop counts often lead to network issues. If you need to increase the hop count, apply changes only to nodes at the network’s edges, not those near the middle.
...
There are 8 total messaging channels. Channel 0 is your PRIMARY channel, with channels 1-7 available for private group messaging and/or special channels such as [`admin`](/docs/2.7/configuration/remote-admin).

### Radio Config: LoRa Frequency Slot

This property, formerly known as "LoRa Channel Number", configures the frequency for the radio. Check out the [frequency calculator](/docs/2.7/overview/radio-settings#frequency-slot-calculator) to view the relationship between "frequency slot" and radio frequency.
...
- Connecting a node to the [public MQTT server](/docs/2.7/configuration/module/mqtt#connect-to-the-default-public-server) may publish the locations of all nodes in your mesh to the internet.

## Keeping things quiet

Things can get noisy in bigger meshes, especially when your device has a buzzer.
This can become problematic at times, particularly in situations involving essential communications in an area with a chatty public channel.

To mute a channel, see: [channel.mute](/docs/2.7/configuration/radio/channels/#is-muted)

For muting particular nodes, try using the python CLI:
```bash
...
If a particular node is bothering you and you don't want to receive their messages at all, try [ignoring a node](/docs/2.7/configuration/radio/lora#ignore-incoming-array). Which can be performed through the various client applications.

Using the python CLI:

```bash
# ignores a node in the NodeDB
meshtastic --set-ignored-node '!xxxxxxxx'

# un-ignores a node in the NodeDB
meshtastic --remove-ignored-node '!xxxxxxxx'
```