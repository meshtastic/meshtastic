...
:::warning
Setting WiFi credentials, or performing any other local administration, over an untrusted WiFi network exposes that traffic to the network. See [Local Administration Over WiFi](/docs/2.7/development/reference/encryption-technical#local-administration-over-wifi) for the trust-boundary details.
:::
