...
This setting applies to the screen on the device only. The apps take their units from the client's locale (phone, desktop, or browser) — see [Android](/docs/2.7/software/android/user/units-and-locale) and [Apple](/docs/2.7/software/apple/user/units-and-locale) — and are unaffected by it.
