...
import PositionPrecisionBits from "@site/versioned_docs/version-2.7/blocks/_position-precision-bits.mdx";
...
:::info
**Channel Settings** (as described on this page) should not be confused with [Modem Preset Settings](/docs/2.7/configuration/radio/lora#modem-preset)

[Modem Preset Settings](/docs/2.7/configuration/radio/lora#modem-preset) contain the modem configuration (frequency settings, spreading factor, bandwidth, etc.) used for the LoRa radio. These settings are identical for all channels and can **not** be unique per channel.
...
The Channel Settings options are: Name, PSK, Downlink Enabled, Uplink Enabled and Mute. Channel settings are embedded in the `Channel` protobuf as a `ChannelSettings` protobuf and sent as an admin message.
...
|    `admin`     | On Secondary channels, the name `admin` (case-sensitive) designates the `admin` channel used to administer nodes over the mesh. Note that this is a Legacy feature, see [Remote Admin](../remote-admin.mdx) for details. |
...
- The public MQTT server [filters out precise positions](https://meshtastic.org/docs/2.7/software/integrations/mqtt/#location-precision-filtering).

<PositionPrecisionBits />

The client applications have implemented different levels of precision giving the user a practical range to choose from. Setting across the full range of integers can be done via the Python CLI. See [Setting Position Precision](/docs/2.7/configuration/radio/channels/#setting-position-precision) for examples on setting different levels of precision using CLI.
...
For configuring gateways, please see [MQTT](/docs/2.7/configuration/module/mqtt)
